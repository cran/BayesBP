...
#'@description Given Bernstein polynomial coeffients to compute partial differential by ages Fhat
...
#'ages<-35:85
#'years<-1988:2007
#'BPFhat_pd_ages(coef,ages,years)
#'@family outer Bernstein polynomial
#'@export BPFhat_pd_ages

BPFhat_pd_ages <- function(coef, ages, years) {
    n <- sqrt(length(coef))
    if (round(n) != n) {
        stop("length(coef) should be square number")
    }
    a.basis <- BPbasis_pd_ages(n - 1, ages, years, n - 1)
