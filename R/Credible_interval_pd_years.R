...
#'@export Credible_interval_pd_years
#'
Credible_interval_pd_years <- function(result, n_cluster = 1, alpha = 0.05) {
    t1 <- Sys.time()
    if (class(result) == "BP2D_result") {
        ...
        clusterExport(cl, c("BPFhat_pd_years", "BPbasis_pd_years", "mapping_to_01",
            "ages", "years"))
        a <- parSapply(cl, unlist(result$store_coefficient, recursive = F), function(x) BPFhat_pd_years(x,
            ages, years))
        stopCluster(cl)
        gc()
